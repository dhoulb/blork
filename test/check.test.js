	...
	describe("type", () => {
		...
		describe("Size types", () => {
			...
			describe("Minimum and maximum size types", () => {
				...
				test("Size types fail correctly", () => {
					...
					expect(() => check(new Map([["a", 2], ["b", 3], ["c", 4], ["d", 5]]), "map{2,3}")).toThrow(
						TypeError
					);
		...
		describe("Array types", () => {
			...
			test("Correct error message", () => {
				...
				expect(() => check(["a", "b", ""], "str+[]")).toThrow(
					"Must be plain array containing non-empty string"
				);
		...
		describe("Tuple types", () => {
			...
			test("Correct error message", () => {
				expect(() => check(true, "[num, str]")).toThrow(
					/Must be plain array tuple like \[finite number, string\]/
				);
			});
		});
		describe("Object types", () => {
			...
			describe("Named checkers", () => {
				...
				test("Correct error message", () => {
					expect(() => check(true, '{ "name": str }')).toThrow(
						/Must be plain object like { "name": string }/
					);
			...
			describe("Mixed checkers (any checkers and named checkers together)", () => {
				...
				test("Object types with any checkers fail correctly", () => {
					expect(() => check({ name: 123, AAA: 1, BBB: 1 }, '{ "name": str, upper: int }')).toThrow(
						TypeError
					); // Name is wrong type.
					expect(() => check({ name: "abc", AAA: "abc" }, '{ "name": int{5,7}, camel: int }')).toThrow(
						TypeError
					); // AAA is wrong type.

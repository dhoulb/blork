// List of rows we skip entirely.
const SKIPS = ["global code"];

// List of function names we change to `function ()` (for consistency).
const ANON_FUNCS = [undefined, "", "<anonymous>"];

// List of file names we blank out (for consistency).
const EVAL_FILES = [undefined, "", "<anonymous>", "debugger eval code", "[native code]", "eval"];
	...
	// e.g. `    at MyClass.myfunc (a/b/myFile.js:28:60)`
	{
		...
		each(matches) {
			...
			if (matches[1]) {
				// Named function.
				return {
					function: ANON_FUNCS.indexOf(matches[1]) >= 0 ? "function ()" : matches[1] + "()",
					file: EVAL_FILES.indexOf(matches[2]) >= 0 ? "" : matches[2],
				...
				return {
					function: "function ()",
					file: EVAL_FILES.indexOf(matches[5]) >= 0 ? "" : matches[5],
		...
		for (let i = 0; i < PARSERS.length; i++) {
			...
			if (PARSERS[i].match.test(stack)) {
				...
				stack.replace(PARSERS[i].match, (...matches) => {
					// Is this a row we should skip entirely (like "global code" in Safari).
					if (SKIPS.indexOf(matches[0]) === -1) {

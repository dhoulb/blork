...
function unnestedIndexOf(str, char, offset = 0) {
	...
	for (let i = offset, l = str.length; i < l; i++) {
		...
		if (cc === ccChar && !stack.length)
			...
			if (stack.pop() !== 40) throw new BlorkError('Invalid nesting (expected ")"', String.fromCharCode(cc));
		} else if (cc === 93) {
			// `]` closing square bracket: pop from stack (throw error if malnested).
			if (stack.pop() !== 91) throw new BlorkError('Invalid nesting (expected "]"', String.fromCharCode(cc));
		} else if (cc === 125) {
			// `}` closing curly bracket: pop from stack (throw error if malnested).
			if (stack.pop() !== 123) throw new BlorkError('Invalid nesting (expected "}"', String.fromCharCode(cc));
		}
	}

...
function modifyChecker(modifiers, type, checker) {
	...
	for (let i = 0; i < l; i++) {
		...
		if (m.start && m.end) {
			...
			if (j === 0 && type.lastIndexOf(m.end) === type.length - m.end.length) return m.callback(type.substring(m.start.length, type.length - m.end.length).trim(), checker);
		} else if (m.start) {
			...
			if (type.lastIndexOf(m.end) === type.length - m.end.length) return m.callback(type.substring(0, type.length - m.end.length).trim(), checker);
		} else if (m.split) {
			// String split, e.g. "|" or "&"

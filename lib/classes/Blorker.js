...
class Blorker {
	 ...
	 * @param {string} description="" A description of the type of value that's valid for this checker, e.g. "positive number" or "unique string". Defaults to same value as name.
	...
	add(name, checker, description = "", error) {
		...
		runChecker(this._checkers.string, description, "add(): description", BlorkError);
	...
	prop(object, name, value, type) {
		// Check args.
		runChecker(this._checkers.objectlike, object, "prop(): object", BlorkError, this);
		runChecker(this._checkers["string+"], name, "prop(): name", BlorkError, this);
		...
		if (arguments.length < 4) {
			if (value instanceof Object) {
				...
				type = Object.getPrototypeOf(value).constructor;
			} else {
			...
			this._check(value, type, "prop(): value", BlorkError);
		}
		...
		Object.defineProperty(object, name, {
			...
			set: v => {
				...
				const prefix = `${constructor.name}.${name}`;

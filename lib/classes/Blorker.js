...
class Blorker {
	...
	check(value, type, prefix = "") {
		...
		this._check(value, type, prefix, this._error);
	}
	...
	args(argsobj, types) {
		...
		for (let i = 0; i < l; i++) this._check(argsobj[i], types[i], `arguments[${i}]`, this._error);
	...
	/**
	 * Define an object property that is locked to a specified Blork type.
	 *
	 * The property is readable, but only writable with values matching the Blork type.
	 ...
	 * @param {mixed} value The value to set for the
	 * @param {string|Function|Array|Object} type={auto} A single stringy type reference (e.g. 'str'), functional shorthand type reference (e.g. `String`), or an object/array with list of types (e.g. `{name:'str'}` or `['str', 'num']`). Alternatively leave the property blank and the type will be inferred from `value`.
	 * @returns {void}
	 */
	prop(object, name, value, type) {
		// Check args.
		runChecker(this._checkers.objectlike, object, "lock(): object", BlorkError, this);
		runChecker(this._checkers["string+"], name, "lock(): name", BlorkError, this);

		// If type is undefined, figure it out from value.
		if (arguments.length < 4) {
			...
			// Check that value corresponds to specified type.
			this._check(value, type, "lock(): value", BlorkError);
		}

		// Figure out prefix.
		// Either "obj.myProp" or "MyRandomClass.myProp"
		const prefix = `${object.constructor.name || "object"}.${name}`;

		// Make a function to do the checking
	 ...
	 * @internal
	 */
	_check(value, type, name, error) {
		// Found.
		if (typeof type === "string") runChecker(this._find(type), value, name, error);
		else if (type === true) runChecker(this._checkers.true, value, name, error);
		else if (type === false) runChecker(this._checkers.false, value, name, error);
		else if (type === null) runChecker(this._checkers.null, value, name, error);
		else if (type === undefined) runChecker(this._checkers.undefined, value, name, error);
		else if (type === Boolean) runChecker(this._checkers.boolean, value, name, error);
		else if (type === Number) runChecker(this._checkers.number, value, name, error);
		else if (type === String) runChecker(this._checkers.string, value, name, error);
			...
			// Pass to checkArray.
			this._checkArray(value, type, name, error);
		} else if (type instanceof Object) {
			// Pass to checkObject.
			this._checkObject(value, type, name, error);
		} else {
			// Not found.
	 ...
	 * @internal
	 */
	_checkArray(value, type, name, error) {
		// Value itself must be an array (check using the array checker).
		runChecker(this._checkers.array, value, name, error);
		...
		if (type.length === 1) {
			...
			for (let i = 0; i < length; i++) this._check(value[i], type[0], `${prefix}[${i}]`, error);
		} else {
			// Tuple array
			// Loop through types and match each with a value recursively.
			const length = type.length;
			for (let i = 0; i < length; i++) this._check(value[i], type[i], `${prefix}[${i}]`, error);
	 ...
	 * @internal
	 */
	_checkObject(value, type, name, error) {
		// Value itself must be an object (check using the object checker).
		runChecker(this._checkers.object, value, name, error);
		...
		for (let i = 0; i < typeKeys.length; i++) {
			...
			if (key !== "_any") {
				...
				this._check(value[key], type[key], prefix, error);
			}
		}
		...
		if (type.hasOwnProperty("_any")) {
			...
			if (valueKeys.length > typeKeys.length - 1) {
				...
				for (let i = 0; i < excessKeys.length; i++) {
					...
					const prefix = name ? `${name}.${key}` : key;
					this._check(value[key], type._any, prefix, error);
				}
			}
		}

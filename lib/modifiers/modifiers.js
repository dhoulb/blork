...
module.exports = [
	...
	// e.g. `arr{4,}` (arrays with a minimum of 4 itmes)
	{
		...
		callback: function sizeModifier(matches, find) {
			...
			const max = matches[2] ? parseInt(matches[2], 10) : matches[4] ? parseInt(matches[4], 10) : matches[5] ? parseInt(matches[5]) : null;
			...
			const checker = v => {
				...
				if (typeof v === "string" || v instanceof Array) return (min === null || v.length >= min) && (max === null || v.length <= max);
				// Map and set use .size
				if (v instanceof Map || v instanceof Set) return (min === null || v.size >= min) && (max === null || v.size <= max);

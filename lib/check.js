 ...
 * @internal
 */
function check(value, type, name, err) {
	// Found.
	if (typeof type === "string") return checkString(value, type, name, err);
	if (type instanceof Function) return checkFunction(value, type, name, err);
	if (type instanceof Array) return checkArray(value, type, name, err);
	if (type instanceof Object) return checkObject(value, type, name, err);
 ...
 * @internal
 */
function checkString(value, type, name, err) {
 ...
 * @internal
 */
function checkFunction(value, type, name, err) {
	// Vars.
	let result = true;
 ...
 * @internal
 */
function checkArray(value, type, name, err) {
	...
	if (value instanceof Array) {
		...
		if (type.length === 1) {
			...
			for (let i = 0; i < l; i++) if (check(value[i], type[0], `${prefix}[${i}]`, err)) pass++;
		} else {
			// Tuple array: Loop through types and match each with a value recursively.
			const l = type.length;
			for (let i = 0; i < l; i++) if (check(value[i], type[i], `${prefix}[${i}]`, err)) pass++;
 ...
 * @internal
 */
function checkObject(value, type, name, err) {
	// Value must be an object.
	if (typeof value !== "object" || value === null) throw new err(format("Must be an object", value, name));

	// Recurse into each type.
	let pass = 0;
	for (const key in type) if (check(value[key], type[key], name ? `${name}[${key}]` : key, err)) pass++;
	return pass;
}

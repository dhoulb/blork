 ...
 * @param {Error} err The error to throw if the checker fails.
 * @return {void}
 */
function run(checker, value, prefix, err) {
	// Check the value. If we fail, throw.
	if (!checker(value)) throw new err(format(`Must be ${checker.desc}`, value, prefix));
}
...
class Blorker {
	...
	add(name, checker, desc = "") {
		...
		// Don't double up.
		if (this._checkers[name]) throw new BlorkError(`Blork type '${name}' already exists`);
	...
	_find(type) {
		...
		else throw new BlorkError(`Blork type '${type}' does not exist`);
	}

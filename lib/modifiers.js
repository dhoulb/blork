...
module.exports = [
	...
	// String literal type, e.g. `"abcdef"` or `'1937'`
	{
		// `"` double or `'` single quote that takes up the entire string with no double or single quotes (respectively) inbetween.
		regex: /^"([^"]*)"|'([^']*)'$/,
		callback(matches) {
			// Create a checker using either it $1 (double quotes) or $2 (single quotes).
			if (typeof matches[1] === "string") {
				// Create a checker.
				const checker = v => typeof v === "string" && v === matches[1];

				// Checker settings.
				checker.desc = `"${matches[1]}"`;

				// Return it.
				return checker;
			} else {
				// Create a checker.
				const checker = v => typeof v === "string" && v === matches[2];

				// Checker settings.
				checker.desc = `'${matches[2]}'`;
